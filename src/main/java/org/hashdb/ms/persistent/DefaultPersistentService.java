...
//            FileUtils.prepareDir(indexFile, () -> new DBFileAccessFailedException("can`t access index db file '" + indexFile.getAbsolutePath() + "'"));
    ...
    public boolean persist(ReplicationConfig config) {
        try {
            ...
            File reConfigFileDir = FileUtils.prepareDir(HDBConfig.getReplicationConfigPath(), (f) ->
                    new DBSystemException("can not open/create the parent dir of replication config file. dir: " + f.getAbsolutePath()));
            try (FileWriter writer = new FileWriter(new File(reConfigFileDir, HDBConfig.getReplicationConfigFileName()))) {
                writer.write(toSave);
            } catch (IOException e) {
                log.error("can not write replication config file ", e);
    ...
    public @NotNull ReplicationConfig scanReplicationConfig() {
        File configFile = new File(HDBConfig.getRootDir(), HDBConfig.getReplicationConfigFileName());
        if (!configFile.exists()) {
            return new ReplicationConfig();
        }
        try {
            return YAML_MAPPER.readValue(new FileReader(configFile), ReplicationConfig.class);
